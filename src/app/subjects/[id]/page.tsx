  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Tag inline display/picker */}
        <div ref={tagRef} className="relative shrink-0 hidden sm:block">
          <button
            onClick={(e) => { e.stopPropagation(); setShowTagPicker(!showTagPicker); }}
            className={cn(
              'rounded-lg transition-all flex items-center shrink-0',
              chapterTag 
                ? cn('px-2 py-1 text-[10px] font-semibold', IMPORTANCE_TAG_CONFIG[chapterTag]?.bg, IMPORTANCE_TAG_CONFIG[chapterTag]?.text)
                : 'p-1.5 text-[hsl(var(--muted-foreground))] opacity-0 group-hover:opacity-100 hover:bg-[hsl(var(--muted))]'
            )}
            title={chapterTag ? IMPORTANCE_TAG_CONFIG[chapterTag]?.description : "Set importance tag"}
          >
            {chapterTag ? (
              IMPORTANCE_TAG_CONFIG[chapterTag]?.label
            ) : (
          ...
          <AnimatePresence>
            {showTagPicker && (
              <motion.div
                ...
                className="absolute right-0 top-full mt-1 z-30 w-52 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] shadow-xl overflow-hidden"
                onClick={(e) => e.stopPropagation()}
              >
                <div className="p-1.5">
                  <p className="text-[9px] font-semibold text-[hsl(var(--muted-foreground))] uppercase tracking-wider px-2 py-1.5">Importance Tag</p>
                  
                  {chapterTag && (
                    <button
                      onClick={() => { setChapterTag(subjectId, chapter.id, null); setShowTagPicker(false); }}
                      className="w-full text-left px-2.5 py-1.5 rounded-lg text-xs flex items-center gap-2 hover:bg-red-500/10 text-red-500 transition-colors mb-1"
                    >
                  ...
                  {Object.entries(IMPORTANCE_TAG_CONFIG).map(([key, config]) => {
                    const isActive = chapterTag === key;
                    return (
                      <button
                        key={key}
                        onClick={() => {
                          setChapterTag(subjectId, chapter.id, key as ImportanceTag);
                          setShowTagPicker(false);
                        }}
                        className={cn(
                          'w-full text-left px-2.5 py-1.5 rounded-lg text-xs flex items-center gap-2 transition-colors',
                          isActive ? config.bg : 'hover:bg-[hsl(var(--muted))]'
                        )}
                      >
                        <span className={cn('w-1.5 h-1.5 rounded-full shrink-0', isActive ? config.text.replace('text-', 'bg-') : 'bg-[hsl(var(--muted-foreground))]')} />
                        <span className={cn('flex-1', isActive && config.text)}>{config.label}</span>
        ...
        {/* Estimated marks display/picker */}
        <div ref={marksRef} className="relative shrink-0 hidden sm:block">
          <button
            onClick={(e) => {
              e.stopPropagation();
              setDraftMarks(estimatedMarks?.toString() || '');
              setShowMarksPicker(!showMarksPicker);
            }}
            className={cn(
              'rounded-lg transition-all flex items-center shrink-0',
              estimatedMarks !== null && estimatedMarks !== undefined
                ? 'px-2 py-1 text-[10px] font-semibold bg-amber-500/10 text-amber-500'
                : 'p-1.5 text-[hsl(var(--muted-foreground))] opacity-0 group-hover:opacity-100 hover:bg-[hsl(var(--muted))]'
            )}
            title={estimatedMarks !== null ? `${estimatedMarks} Estimated Marks` : "Set estimated marks"}
          >
            {estimatedMarks !== null && estimatedMarks !== undefined ? (
          ...
          <AnimatePresence>
            {showMarksPicker && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0, scale: 1 }}
                exit={{ opacity: 0, y: -4, scale: 0.95 }}
                className="absolute right-0 top-full mt-1 z-30 w-48 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] shadow-xl overflow-hidden p-2"
                onClick={(e) => e.stopPropagation()}
              >
                <p className="text-[9px] font-semibold text-[hsl(var(--muted-foreground))] uppercase tracking-wider px-1 pb-1.5">Estimated Marks</p>
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="e.g. 5"
                    className="w-full bg-transparent border border-[hsl(var(--border))] rounded-lg px-2.5 py-1.5 text-xs outline-none focus:border-amber-500 transition-colors"
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        e.stopPropagation();
                        const val = parseInt(draftMarks);
                        updateChapter(subjectId, chapter.id, { estimatedMarks: isNaN(val) ? null : val });
                        setShowMarksPicker(false);
                      }
                    }}
                  />
                  <button
                    ...
                    className="shrink-0 bg-amber-500/10 text-amber-500 hover:bg-amber-500/20 px-2.5 rounded-lg text-xs font-medium transition-colors"
                  >
                    Save
                  </button>
                </div>
                {estimatedMarks !== null && estimatedMarks !== undefined && (
                  <button
                    onClick={() => {
                      updateChapter(subjectId, chapter.id, { estimatedMarks: null });
                      setShowMarksPicker(false);
                    }}
                    className="w-full text-left px-2 py-1.5 mt-1.5 rounded-lg text-[11px] flex items-center gap-2 hover:bg-red-500/10 text-red-500 transition-colors"
                  >
                    <Trash2 className="w-3 h-3 shrink-0" />
                    Clear marks
                  </button>
